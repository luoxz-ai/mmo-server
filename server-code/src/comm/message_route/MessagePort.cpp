	...
	case NETMSG_INTERNAL:
		{
			...
			CNetworkMessage* pMsg = new CNetworkMessage(pBuffer + sizeof(MSG_INTERNAL_MSG_HEAD), pHead->usSize - sizeof(MSG_INTERNAL_MSG_HEAD), pInternal_msg_head->nFrom, pInternal_msg_head->nTo);
			pMsg->CopyBuffer();
	...
	case NETMSG_INTERNAL_FORWARD:
		{
			...
			CNetworkMessage* pMsg = new CNetworkMessage(pBuffer + sizeof(MSG_INTERNAL_FORWARD_MSG_HEAD), pHead->usSize - sizeof(MSG_INTERNAL_FORWARD_MSG_HEAD), pInternal_msg_head->nFrom, pInternal_msg_head->nTo, pInternal_msg_head->nForward);
			pMsg->CopyBuffer();
			m_RecvMsgQueue.push(pMsg);
		}
		break;	
