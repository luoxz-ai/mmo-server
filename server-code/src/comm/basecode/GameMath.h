...
export_lua struct Rect
{
	...
	export_lua long width() const
	{
		return right - left;
	}
	export_lua long height() const
	{
		return bottom - top;
	}
	export_lua inline bool isIntersect(const Vector2& pos, float vDis)
	{
		return (left - vDis <= pos.x && pos.x <= right + vDis &&
			top - vDis <= pos.y && pos.y <= bottom + vDis ) ;
	}
};
...
export_lua struct FloatRect
{
	...
	export_lua float width() const
	{
		return right - left;
	}
	export_lua float height() const
	{
		return bottom - top;
	}
...
export_lua namespace GameMath
{
	...
	{
		return (rect.left - vDis <= pos.x && pos.x <= rect.right + vDis &&
			rect.top - vDis <= pos.y && pos.y <= rect.bottom + vDis ) ;
	}
	...
	export_lua inline Vector2 rotateByRadian(const Vector2& vec, float radian)
	{
		...
		return  Vector2(cosa * vec.x - sina * vec.y,
