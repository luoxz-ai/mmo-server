...
namespace detail
{
    ...
    template<class... Ts> struct overloaded_t : Ts... { using Ts::operator()...; };
...
struct FieldSchemaHelper
{
    ...
    static inline constexpr auto member_ptr_tuple_size()
    {
        return field_tuple_size() - 1;
    }
};
...
inline constexpr void ForEachField_Index(T&& value, Fn&& fn)
{
    ...
    detail::for_each_tuple_index(std::move(struct_schema), [&fn,&value](auto FieldIndex, auto&&)
    {
        ...
        // field_schema = std::tuple<FieldName, std::tuple<MemberPtr>, std::tuple<MemberPtr,Tag>, std::tuple<MemberPtr,Func> >
        ...
        detail::for_each_tuple_index(std::move(field_schema), 
        [&fn,&value, &field_name_tuple]
        (auto TagIndex, const auto&&)
        {
