...
#define CHECK(x)		{ if(!(x)) { LOGASSERT("ASSERT:"#x); return;} }
#define CHECKF(x)		{ if(!(x)) { LOGASSERT("ASSERT:"#x); return 0;} }
#define CHECKFR(x,r)	{ if(!(x)) { LOGASSERT("ASSERT:"#x); return r;} }

#define CHECK_M(x,msg)		{ if(!(x)) { LOGASSERT("ASSERT:"#x" msg:{}",msg); return;} }
#define CHECKF_M(x,msg)		{ if(!(x)) { LOGASSERT("ASSERT:"#x" msg:{}",msg); return 0;} }
#define CHECKFR_M(x,r,msg)	{ if(!(x)) { LOGASSERT("ASSERT:"#x" msg:{}",msg); return r;} }

#define __ENTER_FUNCTION {try{
#define __LEAVE_FUNCTION }catch(const std::runtime_error& e){LOGASSERT("catch_execpetion:{}",e.what());}catch(const std::exception& e){LOGASSERT("catch_execpetion:{}",e.what());}catch(...){LOGASSERT("catch_error");}}
...
export_lua inline bool scan_dir(const std::string& parent_path, const std::string& path, bool bRecursive, std::function<void(const std::string&,const std::string&)> func)
{
