...
public:
	...
	export_lua static void FindTarget(CActor* pOwner, CSkillType* pSkillType, OBJID idTarget, const Vector2& posTarget, std::vector<CActor*>& vecTarget);
	export_lua static void DoMultiDamage(CActor* pOwner, CSkillType* pSkillType, OBJID idTarget, const Vector2& posTarget, const std::vector<CActor*>& vecTarget);

	export_lua static int32_t DoDamage(CActor* pOwner, CSkillType* pSkillType, CActor* pTarget, OBJID idTarget, const Vector2& posTarget);
	export_lua static void	  AttachStatus(CActor* pOwner, CSkillType* pSkillType, const std::vector<CActor*>& vecTarget);
	export_lua static void	  AddBullet(CActor* pOwner, uint32_t idBulletType, OBJID idTarget, const Vector2& posTarget, const std::vector<CActor*>& vecTarget);
	void					  Stop();
