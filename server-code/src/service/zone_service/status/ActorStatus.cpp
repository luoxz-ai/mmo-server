...
__ENTER_FUNCTION
	...
	CHECKF(pDB);
	auto pResult = pDB->Query(TBLD_STATUS::table_name, fmt::format("SELECT * FROM {} WHERE userid={}", TBLD_STATUS::table_name,m_pOwner->GetID()));
	if (pResult)
	{
...
{
__ENTER_FUNCTION
	auto it = std::find_if(m_setStatus.begin(), m_setStatus.end(), [nStatusType](const std::pair<const short unsigned int, CStatus*>& pair_val)->bool
	{
...
{
__ENTER_FUNCTION
	auto it = std::find_if(m_setStatus.begin(), m_setStatus.end(), [nFlag](const std::pair<const short unsigned int, CStatus*>& pair_val)->bool
	{
