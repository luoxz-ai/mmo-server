...
CCommonData::CCommonData(CPlayer* pPlayer) : m_pOwner(pPlayer)
{
...
{
__ENTER_FUNCTION
	CHECKF(0 > nIdx && nIdx < 4); return m_pRecord->Field(TBLD_COMMONDATA::DATA0 + nIdx);
__LEAVE_FUNCTION
	return 0;
}
...
__ENTER_FUNCTION
	...
	auto result = pDB->Query(TBLD_COMMONDATA::table_name, fmt::format(FMT_STRING("SELECT * FROM {} WHERE {}={}"), TBLD_COMMONDATA::table_name,TBLD_COMMONDATA::field_name[TBLD_COMMONDATA::PLAYERID],pPlayer->GetID()));
	if (result)
	{
...
__ENTER_FUNCTION
	...
	for (auto& [k,pData] : m_setData)
	{
		...
		static const int MAX_DATA_PER_MSG = 50;
