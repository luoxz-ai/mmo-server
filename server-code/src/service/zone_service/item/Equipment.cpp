...
__ENTER_FUNCTION
	...
	CHECKF(pDB);
	auto pResult = pDB->Query(TBLD_ITEM::table_name, fmt::format("SELECT * FROM {} WHERE {}={} AND {}={}", TBLD_ITEM::table_name, 
																	TBLD_ITEM::field_name[TBLD_ITEM::OWNER_ID], pPlayer->GetID(), 
																	TBLD_ITEM::field_name[TBLD_ITEM::POSITION], ITEMPOSITION_EQUIP));
...
__ENTER_FUNCTION
	...
	if (nPosition == nGrid ||
		(nPosition == EQUIPPOSITION_RING_L && nPosition == EQUIPPOSITION_RING_R) ||
