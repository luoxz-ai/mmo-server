...
public:
	...
	export_lua void SetTeamID(OBJID val) { m_pRecord->Update(); m_pRecord->Field(TBLD_PLAYER::TEAMID) = val; m_pRecord->ClearModify(); }
	...
	export_lua void SetGuildID(OBJID val) { m_pRecord->Update(); m_pRecord->Field(TBLD_PLAYER::GUILDID) = val; m_pRecord->ClearModify();  }
...
public:
	...
	export_lua bool DialogAddLink(uint32_t nLinkType, const std::string& link_txt,uint32_t idFuncType,  uint64_t idData, const std::string& callback_func, uint64_t idNpc = 0);
