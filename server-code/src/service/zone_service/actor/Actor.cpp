...
__ENTER_FUNCTION
	m_bDelThis = true;
	EventManager()->ScheduleEvent(EVENTID_DELACTOR, [idActor = GetID()]()
	{
		ActorManager()->DelActorByID(idActor);
	}, 0, false, GetEventMapRef());
__LEAVE_FUNCTION
}
...
__ENTER_FUNCTION
	...
	{
		//200毫秒后一次性发送
		EventManager()->ScheduleEvent(EVENTID_SEND_ATTRIB_CHANGE, [pThis = this]()
		{	
			pThis->SendDelayAttribChage();
		}, 200, false, GetEventMapRef());
	}
__LEAVE_FUNCTION
}
...
int32_t CActor::BeAttack(CActor* pAttacker, uint32_t idSkill, uint32_t nHitType, 
