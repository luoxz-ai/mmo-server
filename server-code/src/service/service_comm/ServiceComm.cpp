...
struct INIT_BRPC_LOG
{
	INIT_BRPC_LOG()
	{
		logging::SetLogSink(new BRPCLogSink);
	}
};
...
void CServiceCommon::StartLogicThread(int32_t nWorkInterval, const std::string& name)
{
	...
	m_pLogicThread = std::unique_ptr<CNormalThread>(new CNormalThread(nWorkInterval, name,
...
__ENTER_FUNCTION
	...
	{
		LOGWARNING("Message Want Send To Worng: {} {} {}", msg.GetTo().GetServerPort().GetWorldID(), msg.GetTo().GetServerPort().GetServiceID(), msg.GetTo().GetSocketIdx());
#ifdef DEBUG
