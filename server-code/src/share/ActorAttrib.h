...
public:
	CActorAttribChange(uint32_t nAttribIdx = 0, uint32_t nOpLev = 0, int16_t nValModify = 0, int16_t nValFactor = 0)
		:m_nAttribIdx(nAttribIdx), m_nOpLev(nOpLev), m_AttribChangeVal{ nValModify,nValFactor }
	{		
	}
	CActorAttribChange(const AttribChangeDataProto& v)
		:m_nAttribIdx(v.attrib()), m_nOpLev(v.oplev()), m_AttribChangeVal{ v.val(), v.factor()}
	{}
...
public:
	...
	uint32_t get(size_t nIdx) const { CHECKF(nIdx < ATTRIB_MAX); return m_setAttrib[nIdx]; }
	uint32_t& get(size_t nIdx) { CHECKFR(nIdx < ATTRIB_MAX, m_setAttrib[nIdx]); return m_setAttrib[nIdx]; }
	uint32_t get_base(size_t nIdx) const { CHECKF(nIdx < ATTRIB_MAX); return m_setAttribBase[nIdx]; }
	uint32_t& get_base(size_t nIdx) { CHECKFR(nIdx < ATTRIB_MAX, m_setAttribBase[nIdx]); return m_setAttribBase[nIdx]; }

	void Store(const CActorAttribCalc& valModify_All)
	{
		m_Calc.Merge(valModify_All);
	}
	void StoreTmp(const CActorAttribCalc& valModify_All)
	{
		m_CalcTmp.Merge(valModify_All);
	}
