...
PB_API size_t pb_readvalue(pb_Slice* s, pb_Value* value)
{
	...
	switch(value->wiretype)
	{
		default:
			break;
		case PB_TVARINT:
			ret = pb_readvarint(s, &value->u.fixed64);
			break;
		case PB_T64BIT:
			ret = pb_readfixed64(s, &value->u.fixed64);
			break;
		case PB_TBYTES:
			ret = pb_readslice(s, &value->u.data);
			break;
		case PB_T32BIT:
			ret = pb_readfixed32(s, &value->u.fixed32);
			break;
		case PB_TGSTART:
			ret = pb_readgroup(s, key, &value->u.data);
			break;
	}
	if(!ret)
		s->p = p;
...
PB_API size_t pb_skipvalue(pb_Slice* s, uint32_t key)
{
	...
	switch(pb_gettype(key))
	{
		...
		case PB_T64BIT:
			...
			break;
		case PB_TBYTES:
			ret = pb_readslice(s, &data);
			break;
		case PB_T32BIT:
			ret = pb_skipsize(s, 4);
			break;
		case PB_TGSTART:
			ret = pb_readgroup(s, key, &data);
			break;
	}
	if(!ret)
		s->p = p;
...
PB_API pb_Slice pb_result(pb_Buffer* b)
{
	pb_Slice slice = {b->buff, b->buff + b->size};
	return slice;
}

PB_API void pb_addbytes(pb_Buffer* b, pb_Slice s)
{
	pb_addvar32(b, (uint32_t)pb_slicelen(&s));
	pb_addslice(b, s);
}
...
PB_API pb_Entry* pbM_getentry(pb_Map* m, pb_Entry* entry)
{
	...
	for(; e->hash != entry->hash || strcmp((char*)e->key, (char*)entry->key); e += e->next)
		if(e->next == 0)
			return NULL;
	return e;
}
...
{
	if(ptr)
		*(void**)ptr = pool->freed, pool->freed = ptr;
}
...
{
	pb_Entry* e;
	if(pb_slicelen(&qname) > 0 && *qname.p == '.')
		++qname.p;
...
static int pbL_rawfield(pb_State* S, pb_Type* t, pb_Field* f, pb_Slice name)
{
	...
	{
		pb_Entry* oet = pbM_geti(&t->field_tags, t->is_enum ? fn->u.enum_value : fn->tag);
