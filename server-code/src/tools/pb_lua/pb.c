...
static int luaL_fileresult(lua_State *L, int stat, const char *fname) {
    ...
    lua_pushnil(L);
    if (fname)
        lua_pushfstring(L, "{}: {}", fname, strerror(en));
    else
...
static int typeerror(lua_State *L, int idx, const char *type) {
    lua_pushfstring(L, "{} expected, got {}", type, luaL_typename(L, idx));
...
static void check_type(lua_State *L, int type, pb_Field *f) {
    ...
    if (realtype == type || (type == LUA_TSTRING && realtype == LUA_TUSERDATA
        ...
        return;
    lua_pushfstring(L, "{} expected at field '{}', {} got",
...
static void encode_enum(lua_State *L, pb_Buffer *b, pb_Field *f) {
    ...
    if (type == LUA_TNUMBER) {
        ...
        lua_pushfstring(L, "number/string expected at field '{}', {} got",
...
static int Lpb_encode(lua_State *L) {
    ...
    if (!t) {
        lua_pushnil(L);
        lua_pushfstring(L, "can not find type '{}'", tname.p);
        return 2;
    }
    pb_initbuffer(&b);
...
LUALIB_API int luaopen_libpb_lua(lua_State *L) {
    luaL_Reg libs[] = {
...
LUALIB_API int luaopen_libpb_lua_conv(lua_State *L) {
    luaL_Reg libs[] = {
...
LUALIB_API int luaopen_libpb_lua_io(lua_State *L) {
    luaL_Reg libs[] = {
...
static int Lbuf_add(lua_State *L) {
    ...
    case PB_Tfixed64: case PB_Tint64: case PB_Tuint64: case PB_Tsint64:
        ...
        break;
    default:
        lua_pushfstring(L, "unknown type '{}'", type);
...
LUALIB_API int luaopen_libpb_lua_buffer(lua_State *L) {
    luaL_Reg libs[] = {
        { "__gc", Lbuf_reset },
        { "__len", Lbuf_len },
...
static int type_mismatch(lua_State *L, int type, const char *wt) {
    ...
    return luaL_error(L, "can not convert from {} to {}",
            wt, pb_typename(type));
}
...
static int pb_pushscalar(lua_State *L, pb_Slice *dec, int wiretype, int type) {
    ...
    case PB_TBYTES:
        ...
        if (type >= 0 && (type != PB_Tbytes
                      ...
                      ||  type != PB_Tmessage)) {
            restore_decoder(dec);
            return luaL_error(dec->L, "read string with invalid type: {}",
                    pb_types[type]);
        }
#endif
    ...
    default:
        dec->p = p;
        return luaL_error(L, "unsupported wire type: {}", wiretype);
    }
}
...
LUALIB_API int luaopen_libpb_lua_slice(lua_State *L) {
    luaL_Reg libs[] = {
        { "__gc", Lslice_reset },
        { "__len", Lslice_len },
