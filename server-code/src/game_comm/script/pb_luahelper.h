...
namespace pb_luahelper
{
	...
	struct ProtobufMessageWarp
	{
		...
		{
			pb_util::SetMessageData(m_pMessage, key, val);
		}
	...
	struct ConstRepeatedProtobufMessageWarp
	{
		...
		:m_pMessage(pMessage), m_pFieldDesc(pFieldDesc)
		{
			s_nCount++;
		}
		~ConstRepeatedProtobufMessageWarp()
		{
			s_nCount--;
		}
		size_t size()const
		{
			return m_pMessage->GetReflection()->FieldSize(*m_pMessage, m_pFieldDesc);
		}
