...
bool CSceneObject::UpdateViewList()
{
	...
	// step1: 获取当前广播集范围内的对象
	{
		GetCurrentScene()->foreach_SceneNodeInSight(GetPosX(), GetPosY(),
	...
	if (viewcount_max > 0)
	{
		...
		if (setBCActor.size() < viewcount_max)
		{
			...
			struct comp
			{
				bool operator()(const ACTOR_MAP_BY_DIS_DATA& lft, uint32_t rht) const
				{
					return lft.dis < rht;
				}

				bool operator()(uint32_t lft, const ACTOR_MAP_BY_DIS_DATA& rht) const
				{
					return lft < rht.dis;
				}
