...
CREATE TABLE `tbld_dbinfo` (
  ...
  `db_port` smallint(11) UNSIGNED  NOT NULL COMMENT '游戏数据mysql端口',
...
CREATE TABLE `tbld_serverlist` (
  `worldid` smallint(11) UNSIGNED  NOT NULL COMMENT '游戏服编号',
  ...
  `mergeto` smallint(11) UNSIGNED  NOT NULL COMMENT '已经被合服到哪个服了',
  PRIMARY KEY (`worldid`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `tbld_servicedetail` (
  `worldid` smallint(11) UNSIGNED  NOT NULL COMMENT '游戏服编号',
  `serviceid` smallint(11) UNSIGNED  NOT NULL COMMENT '服务编号',
  ...
  `route_port` smallint(11) UNSIGNED  NOT NULL COMMENT '内网通讯端口',
  `publish_addr` varchar(255) CHARACTER SET utf8 NOT NULL COMMENT '外网地址',
  `publish_port` smallint(11) UNSIGNED  NOT NULL COMMENT '外网端口',
  `debug_port` smallint(11) UNSIGNED  NOT NULL COMMENT 'debug端口',
