...
docker exec -i mysql-global sh -c 'exec mysql -uroot -p"test12345" serverinfo'
echo "select * from tbld_servicedetail where worldid=0;" | docker exec -i mysql-global sh -c 'exec mysql -v -uroot -p"test12345" serverinfo'