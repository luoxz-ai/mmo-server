...
#cmd="mkdir -p server-res/res/db/.cmake && cd server-res/res/db/.cmake && cmake -DGLOBAL_IP=${GLOBAL_IP} -DGLOBAL_PORT_START=${GLOBAL_PORTD_START} -DGLOBAL_OUT_IP=${GLOBAL_OUT_IP} /data/mmorpg/server-res/res/db/init_globalservice.sql.template"
